...
describe('Cobalt Guardian', () => {
	test('Cobalt Guardian gains Divine Shield if a friendly mech spawns', async () => {
		...
		const playerBoard: BoardEntity[] = [
			...
			buildSingleBoardEntity(CardIds.Collectible.Paladin.CobaltGuardian, cards, 1),
			{ ...buildSingleBoardEntity(CardIds.NonCollectible.Neutral.WrathWeaver, cards, 3), taunt: true },
		];
	...
	test('Cobalt Guardian does not gain Divine Shield if an enemy mech spawns', async () => {
		...
		const playerBoard: BoardEntity[] = [
			{ ...buildSingleBoardEntity(CardIds.Collectible.Paladin.CobaltGuardian, cards, 1), health: 2 },
		];
		const playerEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
		const opponentBoard: BoardEntity[] = [
			{ ...buildSingleBoardEntity(CardIds.Collectible.Neutral.Mecharoo, cards, 3), taunt: true },
		];
		const opponentEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
	...
	test('Cobalt Guardian does not gain Divine Shield if a friendly non-mech spawns', async () => {
		...
		const playerBoard: BoardEntity[] = [
			{ ...buildSingleBoardEntity(CardIds.NonCollectible.Neutral.Imprisoner, cards, 2), taunt: true },
			buildSingleBoardEntity(CardIds.Collectible.Paladin.CobaltGuardian, cards, 1),
		];
		const playerEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
		const opponentBoard: BoardEntity[] = [
