...
describe('Selfless Hero', () => {
	test('Another minion get divine shield', async () => {
		...
		const playerBoard: readonly BoardEntity[] = [
			buildBoardEntity(CardIds.Collectible.Paladin.SelflessHero, cards, 1),
			buildBoardEntity(CardIds.Collectible.Neutral.RockpoolHunter, cards, 2),
		];
		const playerEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
		const opponentBoard: readonly BoardEntity[] = [
			{ ...buildBoardEntity(CardIds.Collectible.Warlock.VulgarHomunculus, cards, 3), health: 5 },
		];
	...
	test('Dooes not give divine shield if no valid target', async () => {
		...
		const playerBoard: readonly BoardEntity[] = [
			buildBoardEntity(CardIds.Collectible.Paladin.SelflessHero, cards, 1),
			buildBoardEntity(CardIds.Collectible.Paladin.ShieldedMinibot, cards, 2),
		];
		const playerEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
		const opponentBoard: readonly BoardEntity[] = [
