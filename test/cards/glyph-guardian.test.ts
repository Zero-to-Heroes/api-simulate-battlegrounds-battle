...
describe('Glyph Guardian', () => {
	test('Glyph Guardian attack buff is triggered on attack (normal)', async () => {
		...
		const playerBoard: readonly BoardEntity[] = [
			buildBoardEntity(CardIds.NonCollectible.Mage.GlyphGuardianBATTLEGROUNDS, cards, 1),
	...
	test('Glyph Guardian attack buff is triggered on attack (normal) (second case))', async () => {
		...
		const playerBoard: readonly BoardEntity[] = [
			{ ...buildBoardEntity(CardIds.NonCollectible.Mage.GlyphGuardianBATTLEGROUNDS, cards, 1), attack: 4 },
			buildBoardEntity(CardIds.NonCollectible.Neutral.WrathWeaver, cards, 1),
		];
		const playerEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
		const opponentBoard: readonly BoardEntity[] = [
			{ ...buildBoardEntity(CardIds.Collectible.Warlock.VulgarHomunculus, cards, 3), health: 8 },
		];
		const opponentEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
	...
	test('Glyph Guardian attack buff is triggered on attack (upgraded)', async () => {
		...
		const playerBoard: readonly BoardEntity[] = [
			{
				...buildBoardEntity(CardIds.NonCollectible.Mage.GlyphGuardianTavernBrawl, cards, 1),
				attack: 4,
				health: 1,
			},
			buildBoardEntity(CardIds.NonCollectible.Neutral.WrathWeaver, cards, 1),
		...
		const playerEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
		const opponentBoard: readonly BoardEntity[] = [
			{ ...buildBoardEntity(CardIds.Collectible.Warlock.VulgarHomunculus, cards, 3), health: 12 },
		];
		const opponentEntity: PlayerEntity = { tavernTier: 1 } as PlayerEntity;
