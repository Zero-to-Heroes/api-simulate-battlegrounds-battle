...
describe('Security Rover', () => {
	...
	test('Security Rover spawns a mech when dealt damage (normal)', async () => {
		...
		const playerBoard: BoardEntity[] = [
			buildSingleBoardEntity(CardIds.Collectible.Warrior.SecurityRover, cards, sharedState.currentEntityId++),
		];
	...
	test('Security Rover spawns a mech when dealt damage (upgraded)', async () => {
		...
		const playerBoard: BoardEntity[] = [
			buildSingleBoardEntity(
				CardIds.NonCollectible.Warrior.SecurityRoverTavernBrawl,
				cards,
				sharedState.currentEntityId++,
			),
		];
	...
	test('Security Rover does not spawn a mech when board is full even if another minion dies at the same time', async () => {
		...
		const playerBoard: BoardEntity[] = [
			...
			{
				...buildSingleBoardEntity(
					CardIds.Collectible.Warrior.SecurityRover,
					cards,
					sharedState.currentEntityId++,
				),
				taunt: true,
