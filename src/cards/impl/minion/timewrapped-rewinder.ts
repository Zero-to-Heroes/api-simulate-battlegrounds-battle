...
export const TimewrappedRewinder: AfterHeroDamagedCard = {
	cardIds: [TempCardIds.TimewrappedRewinder, TempCardIds.TimewrappedRewinder_G],
	afterHeroDamaged: (minion: BoardEntity, input: AfterHeroDamagedInput) => {
		input.hero.hpLeft = input.hero.hpLeft + input.damage;
		const mult = minion.cardId === TempCardIds.TimewrappedRewinder_G ? 2 : 1;
